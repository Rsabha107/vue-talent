...
import { pt as h } from "./vue.runtime.esm-bundler-DkHMNSvi.js";
//#endregion
//#region node_modules/vue-filepond/dist/vue-filepond.esm.js
/*!
* vue-filepond v7.0.4
...
*/
var import_filepond = (/* @__PURE__ */ __commonJSMin(((exports, module) => {
	/*!
	* FilePond 4.32.12
	...
	*/
	(function(global, factory) {
		...
		var createStore = function createStore(initialState) {
			var queries = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : [];
			var actions = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : [];
		...
		var createElement = function createElement(tag, className) {
			...
			if (className) if (isSVGElement(tag)) attr(element, "class", className);
		...
		var getChildCount = "children" in (isBrowser() ? createElement("svg") : {}) ? function(el) {
		...
		*/
		var thereYet = function thereYet(position, destination, velocity) {
			var errorMargin = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : .001;
		...
		*/
		var spring = function spring() {
			var _ref = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {}, _ref$stiffness = _ref.stiffness, stiffness = _ref$stiffness === void 0 ? .5 : _ref$stiffness, _ref$damping = _ref.damping, damping = _ref$damping === void 0 ? .75 : _ref$damping, _ref$mass = _ref.mass, mass = _ref$mass === void 0 ? 10 : _ref$mass;
			var target = null;
			var position = null;
			var velocity = 0;
			var resting = false;
			var api = createObject({
				interpolate: function interpolate(ts, skipToEndState) {
					if (resting) return;
		...
		var animations = function animations(_ref) {
			...
			forin(mixinConfig, function(property, animation) {
				...
				}], [viewInternalAPI, viewExternalAPI], viewProps, true);
			...
			return function(store) {
				...
				*/
				var _read = function _read() {
					...
					if (!(ignoreRectUpdate && rect.width && rect.height)) updateRect(rect, element, style);
					var api = {
						root: internalAPI,
						props,
						rect
				...
				*/
				var _write = function _write(ts, frameActions, shouldOptimize) {
					...
					activeMixins.forEach(function(mixin) {
						if (mixin.write(ts) === false) resting = false;
					});
					childViews.filter(function(child) {
						return !!child.element.parentNode;
					}).forEach(function(child) {
						if (!child._write(ts, filterFrameActionsForChild(child, frameActions), shouldOptimize)) resting = false;
					});
				...
				var mixinAPIDefinition = Object.assign({}, sharedAPIDefinition, { rect: { get: function get() {
					return rect;
				} } });
		...
		var createOptionAPI = function createOptionAPI(store, options) {
			...
			forin(options, function(key) {
				obj[key] = {
					...
					set: function set(value) {
						store.dispatch("SET_" + fromCamels(key, "_").toUpperCase(), { value });
					}
				};
			});
			return obj;
		...
		function _AsyncGenerator(gen) {
			...
			this._invoke = send;
			if (typeof gen.return !== "function") this.return = void 0;
		}
			...
			var api = {
				...
				decorateConstructor: function(elements, decorators) {
					...
					for (var i = decorators.length - 1; i >= 0; i--) {
						...
						if (elementsAndFinisher.finisher !== void 0) finishers.push(elementsAndFinisher.finisher);
				...
				toClassDescriptor: function(obj) {
					...
					return {
						elements: this.toElementDescriptors(obj.elements),
						finisher
					};
				},
		...
		function _optionalCallableProperty(obj, name) {
			var value = obj[name];
			if (value !== void 0 && typeof value !== "function") throw new TypeError("Expected '" + name + "' to be a function");
			return value;
		}
		...
		var getThousandsSeparator = function getThousandsSeparator() {
			...
			if (thousandsStringWithSeparator !== 1e3.toString()) return getNonNumeric(thousandsStringWithSeparator)[0];
		...
		var queries = function queries(state) {
			return {
				...
				GET_PANEL_ASPECT_RATIO: function GET_PANEL_ASPECT_RATIO() {
					return /circle/.test(state.options.stylePanelLayout) ? 1 : getNumericAspectRatioFromString(state.options.stylePanelAspectRatio);
				},
		...
		var getBlobFromBase64DataURI = function getBlobFromBase64DataURI(dataURI) {
			...
			return getBlobFromByteStringWithMimeType(getByteStringFromBase64DataURI(dataURI), mimeType);
		};
		...
		var getTranfserIdFromHeader = function getTranfserIdFromHeader(header) {
			if (/x-content-transfer-id:/i.test(header)) return (header.split(":")[1] || "").trim() || null;
			return null;
		};
		...
		var createFileLoader = function createFileLoader(fetchFn) {
			...
			var load = function load() {
				...
				else if (source instanceof Blob) api.fire("load", getFileFromBlob(source, source.name));
				else if (isBase64DataURI(source)) api.fire("load", getFileFromBase64DataURI(source));
				else loadURL(source);
			};
		...
		var createFetchFunction = function createFetchFunction() {
			...
			return function(url, load, error, progress, abort, headers) {
				var request = sendRequest(url, buildURL(apiUrl, action.url), Object.assign({}, action, { responseType: "blob" }));
		...
		var processFileChunked = function processFileChunked(apiUrl, action, name, file, metadata, load, error, progress, abort, transfer, options) {
			...
			var requestTransferId = function requestTransferId(cb) {
				...
				var headers = typeof action.headers === "function" ? action.headers(file, metadata) : Object.assign({}, action.headers, { "Upload-Length": file.size });
				var requestParams = Object.assign({}, action, { headers });
			...
			var requestTransferOffset = function requestTransferOffset(cb) {
				...
				var requestParams = {
					headers: typeof action.headers === "function" ? action.headers(state.serverId) : Object.assign({}, action.headers),
					method: "HEAD"
				};
			...
			var processChunk = function processChunk(chunk) {
				...
				var request = chunk.request = sendRequest(ondata(chunk.data), requestUrl, Object.assign({}, chunkServer, { headers }));
				request.onload = function(xhr) {
			...
			var updateTotalProgress = function updateTotalProgress() {
				...
				progress(true, totalBytesTransfered, chunks.reduce(function(total, chunk) {
					return total + chunk.size;
				}, 0));
			};
			var processChunks = function processChunks() {
				if (chunks.filter(function(chunk) {
		...
		var createFileProcessor = function createFileProcessor(processFn, options) {
			...
			var process = function process(file, metadata) {
				...
				state.perceivedPerformanceUpdater = createPerceivedPerformanceUpdater(function(progress) {
					...
					progressFn();
					if (state.response && state.perceivedProgress === 1 && !state.complete) completeFn();
				}, allowMinimumUploadDuration ? getRandomNumber(750, 1500) : 0);
				state.request = processFn(file, metadata, function(response) {
		...
		var createItem = function createItem() {
			...
			var file = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : null;
			var id = getUniqueId();
			/**
			* Internal item state
			*/
			...
			*/
			...
			var api = Object.assign({
				...
				serverId: { get: function get() {
					return state.serverFileReference;
				} },
				transferId: { get: function get() {
					return state.transferId;
				} },
				status: { get: function get() {
					return state.status;
				} },
				filename: { get: function get() {
					return state.file.name;
				} },
				filenameWithoutExtension: { get: function get() {
				...
				released: { get: function get() {
					return state.released;
				} },
				archive: function archive() {
					return state.archived = true;
				},
				archived: { get: function get() {
		...
		var actions = function actions(dispatch, query, state) {
			return {
				...
				DID_SET_FILES: function DID_SET_FILES(_ref2) {
					...
					var files = (_ref2$value === void 0 ? [] : _ref2$value).map(function(file) {
						return {
				...
				ADD_ITEMS: function ADD_ITEMS(_ref6) {
					...
					var promises = items.filter(function isValidFile(source) {
				...
				ADD_ITEM: function ADD_ITEM(_ref7) {
					...
					var item = createItem(origin, origin === FileOrigin.INPUT ? null : source, options.file);
					...
					item.load(source, createFileLoader(origin === FileOrigin.INPUT ? isString(source) && isExternalURL(source) ? fetch ? createFetchFunction(url, fetch) : fetchBlob : fetchBlob : origin === FileOrigin.LIMBO ? createFetchFunction(url, restore) : createFetchFunction(url, load)), function(file, success, error) {
				...
				COMPLETE_LOAD_ITEM: function COMPLETE_LOAD_ITEM(_ref10) {
					...
					if (query("IS_ASYNC") && state.options.instantUpload) dispatch("REQUEST_ITEM_PROCESSING", { query: item.id });
				},
				...
				REQUEST_ITEM_PROCESSING: getItemByQueryFromState(state, function(item, success, failure) {
					if (!(item.status === ItemStatus.IDLE || item.status === ItemStatus.PROCESSING_ERROR)) {
						...
						if (item.status === ItemStatus.PROCESSING_COMPLETE || item.status === ItemStatus.PROCESSING_REVERT_ERROR) item.revert(createRevertFunction(state.options.server.url, state.options.server.revert), query("GET_FORCE_REVERT")).then(process).catch(function() {});
				...
				PROCESS_ITEM: getItemByQueryFromState(state, function(item, success, failure) {
					...
					if (query("GET_ITEMS_BY_STATUS", ItemStatus.PROCESSING).length === maxParallelUploads) {
					...
					item.onOnce("process-complete", function() {
						...
						if (state.options.instantUpload && item.origin === FileOrigin.LOCAL && isFunction(server.remove)) {
							var noop = function noop() {};
							item.origin = FileOrigin.LIMBO;
							state.options.server.remove(item.source, noop, noop);
						}
						if (query("GET_ITEMS_BY_STATUS", ItemStatus.PROCESSING_COMPLETE).length === state.items.length) dispatch("DID_COMPLETE_ITEM_PROCESSING_ALL");
					});
					...
					}), { allowMinimumUploadDuration: query("GET_ALLOW_MINIMUM_UPLOAD_DURATION") }), function(file, success, error) {
				...
				REMOVE_ITEM: getItemByQueryFromState(state, function(item, success, failure, options) {
					...
					if (item.origin === FileOrigin.LOCAL && server && isFunction(server.remove) && options.remove !== false) {
						...
						if (options.revert && item.origin !== FileOrigin.LOCAL && item.serverId !== null || state.options.chunkUploads && item.file.size > state.options.chunkSize || state.options.chunkUploads && state.options.chunkForce) item.revert(createRevertFunction(state.options.server.url, state.options.server.revert), query("GET_FORCE_REVERT"));
						removeFromView();
					}
				}),
				...
				ABORT_ITEM_PROCESSING: getItemByQueryFromState(state, function(item) {
					...
					item.abortProcessing().then(function() {
						if (state.options.instantUpload) dispatch("REMOVE_ITEM", { query: item.id });
					});
				}),
				REQUEST_REVERT_ITEM_PROCESSING: getItemByQueryFromState(state, function(item) {
					...
					if (requestRemoveResult == null) return handleRevert(true);
				...
				REVERT_ITEM_PROCESSING: getItemByQueryFromState(state, function(item) {
					item.revert(createRevertFunction(state.options.server.url, state.options.server.revert), query("GET_FORCE_REVERT")).then(function() {
						if (state.options.instantUpload || isMockItem(item)) dispatch("REMOVE_ITEM", { query: item.id });
					}).catch(function() {});
				}),
				SET_OPTIONS: function SET_OPTIONS(_ref11) {
					...
					[].concat(_toConsumableArray(prioritizedOptionKeys), _toConsumableArray(Object.keys(options).filter(function(key) {
						return !prioritizedOptionKeys.includes(key);
					}))).forEach(function(key) {
						dispatch("SET_" + fromCamels(key, "_").toUpperCase(), { value: options[key] });
					});
				}
			};
		};
		...
		var percentageArc = function percentageArc(x, y, radius, from, to) {
			var arcSweep = 1;
			if (to > from && to - from <= .5) arcSweep = 0;
			if (from > to && from - to >= .5) arcSweep = 0;
			return describeArc(x, y, radius, Math.min(.9999, from) * 360, Math.min(.9999, to) * 360, arcSweep);
		};
		var progressIndicator = createView({
			...
			write: function write(_ref2) {
				...
				var size = root.rect.element.width * .5;
				var ringFrom = 0;
				var ringTo = 0;
				if (props.spin) {
					ringFrom = 0;
		...
		*/
		...
		var calculateButtonWidth = function calculateButtonWidth(root) {
			return root.ref.buttonAbortItemLoad.rect.element.width;
		};
		...
		*/
		var create$4 = function create(_ref2) {
			...
			var buttonFilter;
			if (isAsync) {
				if (allowProcess && !allowRevert) buttonFilter = function buttonFilter(key) {
					return !/RevertItemProcessing/.test(key);
				};
				else if (!allowProcess && allowRevert) buttonFilter = function buttonFilter(key) {
					return !/ProcessItem|RetryItemProcessing|AbortItemProcessing/.test(key);
				};
				else if (!allowProcess && !allowRevert) buttonFilter = function buttonFilter(key) {
					return !/Process/.test(key);
				};
			} else buttonFilter = function buttonFilter(key) {
				return !/Process/.test(key);
			};
		...
		var fileWrapper = createView({
			...
			ignoreRect: true,
			write: createRoute({ DID_LOAD_ITEM: function didLoadItem(_ref2) {
				var root = _ref2.root, props = _ref2.props;
				text(root.ref.fileName, formatFilename(root.query("GET_ITEM_NAME", props.id)));
			} }),
			didCreateView: function didCreateView(root) {
		...
		*/
		var create$7 = function create(_ref) {
			...
			root.element.addEventListener("pointerdown", function grab(e) {
				...
				var drag = function drag(e) {
					...
					if (props.dragOffset.x * props.dragOffset.x + props.dragOffset.y * props.dragOffset.y > 16 && !removedActivateListener) {
		...
		var route$1 = createRoute({ DID_UPDATE_PANEL_HEIGHT: function DID_UPDATE_PANEL_HEIGHT(_ref2) {
			var root = _ref2.root;
			root.height = _ref2.action.height;
		} });
		var item = createView({
			create: create$7,
			write: createRoute({
		...
		var getItemIndexByPosition = function getItemIndexByPosition(view, children, positionInView) {
			...
			var itemRect = children[0].rect.element;
			...
			if (itemsPerRow === 1) {
				for (var index = 0; index < l; index++) {
					...
					var childMid = child.rect.outer.top + child.rect.element.height * .5;
			...
			for (var _index = 0; _index < l; _index++) {
				...
				if (positionInView.top < itemBottom && positionInView.top > itemTop) if (positionInView.left < itemRight) return _index;
		...
		*/
		var addItemView = function addItemView(_ref2) {
			...
			root.appendChildView(root.createChildView(item, {
				spawnDate,
				id,
				opacity,
				interactionMethod
		...
		*/
		...
		var getItemWidth = function getItemWidth(child) {
			return child.rect.element.width + child.rect.element.marginLeft * .5 + child.rect.element.marginRight * .5;
		};
		/**
		* Setup action routes
		*/
		var route$2 = createRoute({
			...
			DID_DRAG_ITEM: function dragItem(_ref4) {
				...
				dropAreaDimensions.setHeight = dragHeight * Math.floor(numItems / cols + 1);
		...
		var list = createView({
				...
				if (itemsPerRow === 1) {
					...
					children.forEach(function(child, index) {
						if (dragIndex) {
							...
							if (dist === -2) dragOffset = -itemVerticalMargin * .25;
							else if (dist === -1) dragOffset = -itemVerticalMargin * .75;
							else if (dist === 0) dragOffset = itemVerticalMargin * .75;
							else if (dist === 1) dragOffset = itemVerticalMargin * .25;
							else dragOffset = 0;
						}
						if (shouldOptimize) {
						...
						var visualHeight = (child.rect.element.height + itemVerticalMargin) * (child.markedForRemoval ? child.opacity : 1);
					...
					children.forEach(function(child, index) {
						...
						if (child.markedForRemoval && child.opacity < .5) removeIndexOffset -= 1;
			...
			filterFrameActionsForChild: function filterSetItemActions(child, actions) {
		...
		var route$3 = createRoute({
			DID_DRAG: function storeDragCoordinates(_ref2) {
			...
			DID_END_DRAG: function clearDragCoordinates(_ref3) {
		...
		var toggleRequired = function toggleRequired(_ref6) {
			var root = _ref6.root;
			if (!_ref6.action.value) attrToggle(root.element, "required", false);
		...
		var updateRequiredStatus = function updateRequiredStatus(_ref8) {
			...
			if (root.query("GET_TOTAL_ITEMS") > 0) {
				...
				for (var i = 0; i < activeItems.length; i++) if (activeItems[i].status === ItemStatus.LOAD_ERROR) hasInvalidField = true;
				root.element.setCustomValidity(hasInvalidField ? root.query("GET_LABEL_INVALID_FIELD") : "");
			} else {
				attrToggle(element, "name", true, root.query("GET_NAME"));
				if (root.query("GET_CHECK_VALIDITY")) element.setCustomValidity("");
		...
		var browser = createView({
			...
			write: createRoute({
				...
				DID_THROW_ITEM_INVALID: function updateFieldValidityStatus(_ref9) {
					var root = _ref9.root;
					if (!root.query("GET_CHECK_VALIDITY")) return;
		...
		var create$b = function create(_ref) {
			...
			root.ref.handleKeyDown = function(e) {
				if (!(e.keyCode === Key.ENTER || e.keyCode === Key.SPACE)) return;
				e.preventDefault();
		...
		var dropLabel = createView({
			...
			write: createRoute({ DID_SET_LABEL_IDLE: function DID_SET_LABEL_IDLE(_ref3) {
				var root = _ref3.root, action = _ref3.action;
		...
		var addBlob = function addBlob(_ref) {
			var root = _ref.root;
			var centerX = root.rect.element.width * .5;
			var centerY = root.rect.element.height * .5;
		...
		var setInputFiles = function setInputFiles(element, files) {
			try {
				...
				files.forEach(function(file) {
					...
					else dataTransfer.items.add(new File([file], file.name, { type: file.type }));
				});
		...
		var data = createView({
			...
			create: create$c,
			write: createRoute({
				DID_SET_DISABLED: function didSetDisabled(_ref6) {
					var root = _ref6.root;
					root.element.disabled = root.query("GET_DISABLED");
				},
				DID_ADD_ITEM: function didAddItem(_ref3) {
					var root = _ref3.root, action = _ref3.action;
					var shouldUseFileInput = !(root.query("GET_ITEM", action.id).origin === FileOrigin.LOCAL) && root.query("SHOULD_UPDATE_FILE_INPUT");
				...
				DID_LOAD_ITEM: function didLoadItem(_ref4) {
					var root = _ref4.root, action = _ref4.action;
					var field = getField(root, action.id);
					if (!field) return;
					if (action.serverFileReference !== null) field.value = action.serverFileReference;
					if (!root.query("SHOULD_UPDATE_FILE_INPUT")) return;
					setInputFiles(field, [root.query("GET_ITEM", action.id).file]);
				},
				DID_REMOVE_ITEM: function didRemoveItem(_ref7) {
					var root = _ref7.root, action = _ref7.action;
					var field = getField(root, action.id);
					if (!field) return;
				...
				DID_DEFINE_VALUE: function didDefineValue(_ref8) {
					var root = _ref8.root, action = _ref8.action;
					var field = getField(root, action.id);
					if (!field) return;
					if (action.value === null) field.removeAttribute("value");
					else if (field.type != "file") field.value = action.value;
					syncFieldPositionsWithItems(root);
				},
				DID_PREPARE_OUTPUT: function didPrepareOutput(_ref5) {
					var root = _ref5.root, action = _ref5.action;
					if (!root.query("SHOULD_UPDATE_FILE_INPUT")) return;
					setTimeout(function() {
						var field = getField(root, action.id);
		...
		*/
		var getFiles = function getFiles(dataTransfer) {
			return new Promise(function(resolve, reject) {
				var promisedFiles = (dataTransfer.items ? Array.from(dataTransfer.items) : []).filter(function(item) {
		...
		*/
		...
		var isEventTarget = function isEventTarget(e, target) {
			var elementAtPosition = elementFromPoint(getRootNode(target), {
		...
		var handlePaste = function handlePaste(e) {
			...
			if (activeEl && (/textarea|input/i.test(activeEl.nodeName) || activeEl.getAttribute("contenteditable") === "true" || activeEl.getAttribute("contenteditable") === "")) {
				var inScope = false;
		...
		*/
		...
		var itemProcessed = function itemProcessed(_ref4) {
			var root = _ref4.root, action = _ref4.action;
			var filename = root.query("GET_ITEM", action.id).filename;
		...
		var write$9 = function write(_ref3) {
			...
			var atMaxCapacity = totalItems === (isMultiItem ? root.query("GET_MAX_FILES") || MAX_FILES_LIMIT : 1);
			...
			if (atMaxCapacity && addAction) {
				...
				else if (interactionMethod === InteractionMethod.API) label.translateX = 40;
		...
		var calculateListHeight = function calculateListHeight(root) {
			...
			var removedItem = children.find(function(child) {
				return child.markedForRemoval && child.opacity < .45;
			}) ? -1 : 0;
		...
		var calculateRootBoundingBoxHeight = function calculateRootBoundingBoxHeight(root) {
			...
			return {
				cappedHeight: parseInt(root.style.maxHeight, 10) || null,
		...
		var exceedsMaxFiles = function exceedsMaxFiles(root, items) {
			...
			if (!allowMultiple && allowReplace) return false;
			if (isInt(maxItems) && totalItems + totalBrowseItems > maxItems) {
				root.dispatch("DID_THROW_MAX_FILES", {
					source: items,
					error: createResponse("warning", 0, "Max files")
				});
		...
		var toggleDrop = function toggleDrop(root) {
			...
			if (enabled && !root.ref.hopper) {
				var hopper = createHopper(root.element, function(items) {
					...
					return root.query("GET_DROP_VALIDATION") ? items.every(function(item) {
						return applyFilters("ALLOW_HOPPER_ITEM", item, { query: root.query }).every(function(result) {
							return result === true;
						}) && beforeDropFile(item);
					}) : true;
				}, {
				...
				hopper.onload = function(items, position) {
					var visibleChildren = root.ref.list.childViews[0].childViews.filter(function(child) {
						return child.rect.element.height;
					});
					var children = root.query("GET_ACTIVE_ITEMS").map(function(item) {
						return visibleChildren.find(function(child) {
		...
		*/
		var toggleBrowse = function toggleBrowse(root, props) {
			...
			var isDisabled = root.query("GET_DISABLED");
			var enabled = isAllowed && !isDisabled;
			if (enabled && !root.ref.browser) root.ref.browser = root.appendChildView(root.createChildView(browser, Object.assign({}, props, { onload: function onload(items) {
				applyFilterChain("ADD_ITEMS", items, { dispatch: root.dispatch }).then(function(queue) {
					if (exceedsMaxFiles(root, queue)) return false;
					root.dispatch("ADD_ITEMS", {
						items: queue,
		...
		*/
		var route$5 = createRoute({
			...
			DID_SET_DISABLED: function DID_SET_DISABLED(_ref8) {
				...
				togglePaste(root);
				toggleBrowse(root, props);
				if (root.query("GET_DISABLED")) root.element.dataset.disabled = "disabled";
				else root.element.removeAttribute("data-disabled");
			}
		});
		var root = createView({
			name: "root",
		...
		var createApp = function createApp() {
			...
			var store = createStore(createInitialState(defaultOptions), [queries, createOptionQueries(defaultOptions)], [actions, createOptionActions(defaultOptions)]);
			...
			var readWriteApi = {
				...
				_write: function _write(ts) {
					var actions = store.processActionQueue().filter(function(action) {
			...
			var exposeEvent = function exposeEvent(event) {
				var detail = Object.assign({ pond: exports$2 }, event);
				delete detail.type;
				...
				exports$2.fire.apply(exports$2, [event.type].concat(params));
			...
			var addFile = function addFile(source) {
				...
				return new Promise(function(resolve, reject) {
					addFiles([{
						source,
						options
					}], { index: options.index }).then(function(items) {
			...
			var exports$2 = Object.assign({}, on(), {}, readWriteApi, {}, createOptionAPI(store, defaultOptions), {
				setOptions,
				addFile,
				addFiles,
				getFile,
				...
				destroy: function destroy() {
					exports$2.fire("destroy", view.element);
		...
		var createAppObject = function createAppObject() {
			...
			return createApp(Object.assign({}, defaultOptions, {}, customOptions));
		};
		...
		var mapObject = function mapObject(object, propertyMap) {
			forin(propertyMap, function(selector, mapping) {
				forin(object, function(property, value) {
					...
					if (!selectorRegExp.test(property)) return;
		...
		*/
		var fn = function fn() {};
		exports$1.Status = {};
		exports$1.FileStatus = {};
		exports$1.FileOrigin = {};
		exports$1.OptionTypes = {};
		exports$1.create = fn;
		exports$1.destroy = fn;
		exports$1.parse = fn;
		exports$1.find = fn;
		exports$1.registerPlugin = fn;
		exports$1.getOptions = fn;
		exports$1.setOptions = fn;
		if (supported()) {
			...
			var dispatch = function dispatch() {
				document.dispatchEvent(new CustomEvent("FilePond:loaded", { detail: {
					supported,
					create: exports$1.create,
					destroy: exports$1.destroy,
					parse: exports$1.parse,
					find: exports$1.find,
					registerPlugin: exports$1.registerPlugin,
					setOptions: exports$1.setOptions
				} }));
			...
			var updateOptionTypes = function updateOptionTypes() {
				return forin(getOptions(), function(key, value) {
					exports$1.OptionTypes[key] = value[1];
				});
			};
			exports$1.Status = Object.assign({}, Status);
			exports$1.FileOrigin = Object.assign({}, FileOrigin);
			exports$1.FileStatus = Object.assign({}, ItemStatus);
			exports$1.OptionTypes = {};
			updateOptionTypes();
			exports$1.create = function create() {
				var app = createApp$1.apply(void 0, arguments);
				app.on("destroy", exports$1.destroy);
				state.apps.push(app);
				return createAppAPI(app);
			};
			exports$1.destroy = function destroy(hook) {
				var indexToRemove = state.apps.findIndex(function(app) {
					return app.isAttachedTo(hook);
				});
				if (indexToRemove >= 0) {
					state.apps.splice(indexToRemove, 1)[0].restoreElement();
					return true;
				}
				return false;
			};
			exports$1.parse = function parse(context) {
				return Array.from(context.querySelectorAll("." + name)).filter(function(newHook) {
					return !state.apps.find(function(app) {
						return app.isAttachedTo(newHook);
					});
				}).map(function(hook) {
					return exports$1.create(hook);
				});
			};
			exports$1.find = function find(hook) {
			...
			exports$1.setOptions = function setOptions$1(opts) {
				if (isObject(opts)) {
					state.apps.forEach(function(app) {
						app.setOptions(opts);
					});
...
var vue_filepond_esm_default = (...plugins) => {
	(0, import_filepond.registerPlugin)(...plugins);
	events.length = 0;
	for (const prop in import_filepond.OptionTypes) {
		if (/^on/.test(prop)) {
			events.push(prop);
			continue;
		}
		let valid_types = [String, getNativeConstructorFromType(import_filepond.OptionTypes[prop])];
		if (prop == "labelFileProcessingError") valid_types.push(Function);
		props[prop] = {
			type: valid_types,
			default: void 0
		};
	...
	return {
		...
		render() {
			...
			return h("div", { class: { "filepond--wrapper": true } }, [h("input", attributes)]);
		},
		created() {
		...
		mounted() {
			...
			this._pond = (0, import_filepond.create)(this._element, Object.assign({}, globalOptions, options, passedProps));
...
//#endregion
export { vue_filepond_esm_default as default, setOptions };

//# sourceMappingURL=vue-filepond_dist_vue-filepond__esm__js.js.map