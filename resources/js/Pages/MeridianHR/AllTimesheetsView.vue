...
const summary = computed(() => {
  ...
  // Calculate worked days using same logic as backend
  let worked = 0
  if (activeTimesheet.value) {
    const startDay = activeTimesheet.value.startDay || 1
    const endDay = activeTimesheet.value.endDay || activeTimesheet.value.daysInMonth
    const daysInMonth = activeTimesheet.value.daysInMonth || 30
    const monthNumber = activeTimesheet.value.monthNumber
    
    // Determine if full month (covers substantially the whole month)
    const isFullMonth = (startDay <= 5 && endDay >= daysInMonth - 2)
    
    if (isFullMonth) {
      // Full month: use actual days for February, 30 for other months
      const baseDays = (monthNumber === 2) ? daysInMonth : 30
      worked = baseDays - leave - unpaid
    } else {
      // Partial month: calendar days - leave - unpaid
      const calendarDays = endDay - startDay + 1
      worked = calendarDays - leave - unpaid
    }
  }
      ...
      <div class="mhr-page-head">
        ...
        <div class="mhr-page-head__actions">
          <button class="mhr-btn mhr-btn--ghost" @click="backToList">
            {{ isApprovedReadOnly ? 'Close' : 'Cancel' }}
          </button>
          <button 
