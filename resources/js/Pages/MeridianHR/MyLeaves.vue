...
// Check if all required fields are filled (Add modal)
...
function openAddModal() {
  // Always auto-set employee_id for personal view
...
function resetAddForm() {
  ...
  // Always restore employee_id for personal view
  if (props.currentEmployee) {
    form.employee_id = props.currentEmployee.id
  }
}
...
<template>
  <div @click="openMenuId = null">
    <div class="mhr-page-head">
      <div>
        <h1 class="mhr-page-head__title">My Leaves</h1>
        <p class="mhr-page-head__sub">Manage your leave requests</p>
      </div>
      <div style="display:flex;gap:8px;align-items:center;margin-left:auto;">
        ...
        <button 
          ...
          :style="!selectedEventId ? 'opacity: 0.5; cursor: not-allowed;' : ''"
          :title="!selectedEventId ? 'Please select an event to create leave requests' : ''"
          @click="openAddModal"
        >
    ...
    <div v-if="showAddModal" class="mhr-modal__scrim" @click.self="closeAddModal">
      <div class="mhr-modal mhr-modal--lg">
        ...
        <div class="mhr-modal__body" style="max-height:70vh;overflow-y:auto;">
          <!-- Error Banner -->
          <div v-if="form.errors.date_from" class="ts-field-error" style="margin-bottom:16px;padding:12px;background:rgba(220,38,38,0.1);border:1px solid var(--mhr-danger);border-radius:var(--mhr-r);">
            {{ form.errors.date_from }}
          </div>
          
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            <!-- Employee field: Always read-only in personal view -->
            ...
            <div class="mhr-field">
              ...
              <select class="mhr-select" v-model="form.status_id" disabled style="cursor:not-allowed;opacity:0.6;">
        ...
        <div class="mhr-modal__ft">
          ...
          <button 
            ...
            :disabled="form.processing || !isAddFormValid"
            :style="(form.processing || !isAddFormValid) ? 'opacity:0.6;cursor:not-allowed;' : ''"
          >
    ...
    <div v-if="showEditModal" class="mhr-modal__scrim" @click.self="closeEditModal">
      <div class="mhr-modal mhr-modal--lg">
        ...
        <div class="mhr-modal__body" style="max-height:70vh;overflow-y:auto;">
          <!-- Error Banner -->
          <div v-if="editForm.errors.date_from" class="ts-field-error" style="margin-bottom:16px;padding:12px;background:rgba(220,38,38,0.1);border:1px solid var(--mhr-danger);border-radius:var(--mhr-r);">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            ...
            <div class="mhr-field">
              <label class="mhr-field__label">STATUS *</label>
              <select class="mhr-select" v-model="editForm.status_id" disabled style="cursor:not-allowed;opacity:0.6;">
                <option :value="null">Select status...</option>
                <option v-for="status in statuses" :key="status.id" :value="status.id">
                  {{ status.title }}
                </option>
